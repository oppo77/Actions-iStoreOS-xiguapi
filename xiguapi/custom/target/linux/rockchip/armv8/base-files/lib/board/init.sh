...
board_fixup_iface_name() {
    ...
    case $(board_name) in
    nlnet,xiguapi-v3)
        # 根据设备树，两个网口都是RTL8211F，通过rgmii_phy0和rgmii_phy1连接
        # 设备树中gmac0对应ethernet0，gmac1对应ethernet1
        # 这里不需要重命名，保持默认顺序即可
        # eth0 = gmac0 (rgmii_phy0)
        # eth1 = gmac1 (rgmii_phy1)
        ;;
...
board_set_iface_smp_affinity() {
    case $(board_name) in
    nlnet,xiguapi-v3)
        # 两个RTL8211F千兆网口，使用内置GMAC控制器
        # 将两个网口的中断分配到不同的CPU核心以提高性能
        # eth0使用CPU0和CPU1 (核心掩码3)
        # eth1使用CPU2和CPU3 (核心掩码c)
        set_iface_cpumask 3 eth0 "" 9
        set_iface_cpumask c eth1 "" 6
        ;;
    inspur,ihec301|\
    ezpro,mrkaio-m68s|\
    firefly,rk3568-roc-pc)
